...
# MAGIC
# MAGIC - Documentation: Gradient-Boosted Trees (GBT) algorithm
# MAGIC
...
# MAGIC %md
# MAGIC This dataset contains the hourly and daily count of rental bikes between years 2011 and 2012 in Capital bikeshare system with the corresponding weather and seasonal information.
...
# MAGIC - `GBTRegressor`: Uses the Gradient-Boosted Trees (GBT) algorithm to learn how to predict rental counts from the feature vectors
# MAGIC - `CrossValidator`: Evaluate and tune ML model. hyperparameter tuning in Spark
...
predictions_with_residuals = predictions.withColumn("residual", (F.col('cnt') - F.col("predictions")))
display(predictions_with_residuals.agg({
    'residual': 'mean'
}))
...
# MAGIC
# MAGIC - For efficiency, we used only a few hyperparameter settings. You might be able to improve the model by testing more settings. A good start is to increase the number of trees by setting maxIter=200; this takes longer to train but might more accurate.
# MAGIC
# MAGIC - We used the dataset features as-is, but you might be able to improve performance with some feature engineering.
# MAGIC For example, the weather might have more of an impact on the number of rentals on weekends and holidays than on workdays. You could try creating a new feature by combining those two columns. MLlib provides a suite of feature transformers; find out more in the ML guide.
